...
- If a review or audit slice finds product defects but the review itself completed successfully, finish with `task_update(state: Reviewed, ...)`, clearly list the defects, and recommend concrete follow-up slices; reserve `Blocked` for cases where the review could not be completed because of missing context, unavailable dependencies, or a truly unreproducible environment problem.
- If the current slice is implemented and focused validation passes, do not mark it `Blocked` only because unrelated baseline packages outside the slice still fail; summarize the unrelated failures, recommend or reference a repair slice, and finish `Done` unless the slice itself cannot be validated at the package boundary it changed.
