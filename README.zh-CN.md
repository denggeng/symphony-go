...
本地任务 front matter 也支持 `priority`、`order`、`depends_on`、`lane`、`review_of`，可以显式控制派发顺序。对于 review 比较多的队列，建议把 `Reviewed` 也加入成功终态；如果你想让“一个实现 worker + 一个 reviewer”并行，还可以配合 concurrency lane 与每任务 git 审查工件一起使用。详细说明见 `docs/zh-CN/local-tasks.md`。
